...
event (root)
  └── coordinate
        └── plan
              ...
              ├── workflow/swe (task 2) → commit → resolve
...
Pushes completed work to the remote repository and creates pull requests. After pushing, it chains to Notify so the source channel is informed about the deliverable.

| | |
|---|---|
| **Trigger** | Pending `push` action |
| **Input** | Push action meta (branch names, trace context) |
| **Span** | `step: "push"`, meta contains PR URL, push status |
| **Actions** | One `notify` action |

**Behavior**:

1. Merge branches from parallel workflow tasks if needed.
2. Push the branch to the remote.
3. Create a pull request with a summary derived from the trace.
4. Record the PR URL and push status in the span.
5. Produce a `notify` action — the notify step will use the PR URL and trace context to post a response on the source channel.
