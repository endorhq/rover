...
Read the `plan` input (scope + constraints) and walk through the `traces` array. Start from the root trace to understand the source event — the original GitHub issue, pull request, comment, or CI result that entered the pipeline. Then read any intermediate coordinate traces to understand how the Coordinator narrowed the scope. Finally, read the plan's scope and constraints to understand the boundaries for your work.
