/**
 * Evaluate "until" conditions for loop steps.
 ...
 *
 * @param condition - e.g. "steps.run_tests.outputs.exit_code == 0"
...
export function evaluateCondition(
  ...
  if (!match) {
    console.warn(
      `Warning: condition "${condition}" does not match expected format "steps.<id>.outputs.<name> == <value>"`
    );
    return false;
  }
