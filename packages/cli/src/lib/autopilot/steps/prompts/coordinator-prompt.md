...
#### GitLab command examples

```
glab mr list --repo GROUP/PROJECT
glab mr view <NUMBER> --repo GROUP/PROJECT
glab mr note list <NUMBER> --repo GROUP/PROJECT
glab issue list --repo GROUP/PROJECT
glab issue view <NUMBER> --repo GROUP/PROJECT
glab ci list --repo GROUP/PROJECT
glab ci view <PIPELINE_ID> --repo GROUP/PROJECT
```

#### Strictly read-only

Allowed: `gh`, `glab`, `git log/show/diff/branch --list`, `qmd`, `rover autopilot inspect`.
...
- **`gh`** — Fetch GitHub issue/PR details, comments, reviews, CI status, labels.
- **`glab`** — Fetch GitLab merge request/issue details, notes, pipelines, CI status.
