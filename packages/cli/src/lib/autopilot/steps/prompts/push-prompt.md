...
- NEVER add AI-generated attribution lines such as "Generated by Claude Code", "Generated with Codex", "Co-Authored-By: Claude", or similar to commit messages, PR titles, or PR bodies. The only attribution allowed is the Rover trailer when present in existing commits. Exception: when the "Required Footer" section provides a `<details>` block, you MUST include it verbatim at the end of the PR body — this is a system tracking footer, not AI attribution.
