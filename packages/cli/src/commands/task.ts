  ...
  if (agent && agent.length > 0) {
    ...
    for (const agentString of agent) {
      try {
        ...
        // If no model specified in CLI, try to get from user settings
        if (!parsed.model) {
          parsed.model = getUserDefaultModel(parsed.agent);
        }
    ...
    try {
      ...
      const defaultModel = getUserDefaultModel(defaultAgent);
      selectedAgents = [{ agent: defaultAgent, model: defaultModel }];
    } catch (_err) {
      if (!json) {
        console.log(
