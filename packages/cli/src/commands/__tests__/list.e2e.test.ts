...
`
  ...
  // describe('hook triggering on status change', () => {
  ...
  //     const roverConfig = JSON.parse(readFileSync(join(testDir, 'rover.json'), 'utf8'));
  //     roverConfig.hooks = {
  //       onComplete: [`echo "TASK_ID=$ROVER_TASK_ID STATUS=$ROVER_TASK_STATUS" > ${hookLogFile}`],
  //     };
  //     writeFileSync(join(testDir, 'rover.json'), JSON.stringify(roverConfig, null, 2));
  ...
  //         if (tasks.length > 0 && (tasks[0].status === 'COMPLETED' || tasks[0].status === 'FAILED')) {
  //           taskCompleted = true;
  //           break;
  //         }
  //       }
  ...
  //     }
  //     if (taskCompleted && existsSync(hookLogFile)) {
