...
import { getRepoInfo } from '../repo-info.js';
...
describe('getRepoInfo', () => {
  ...
  describe('SSH protocol URLs', () => {
    it('parses ssh:// GitHub URL', () => {
      expect(getRepoInfo('ssh://git@github.com/org/repo.git')).toEqual({
        provider: 'github',
        host: 'github.com',
        projectPath: 'org/repo',
      });
    ...
    it('parses ssh:// GitLab URL', () => {
      expect(getRepoInfo('ssh://git@gitlab.com/org/repo.git')).toEqual({
        provider: 'gitlab',
        host: 'gitlab.com',
        projectPath: 'org/repo',
      });
    ...
    it('parses ssh:// GitLab URL with nested groups', () => {
      expect(
        getRepoInfo('ssh://git@gitlab.com/group/subgroup/repo.git')
      ).toEqual({
        provider: 'gitlab',
        host: 'gitlab.com',
        projectPath: 'group/subgroup/repo',
    ...
    it('parses ssh:// URL with port', () => {
      expect(
        getRepoInfo('ssh://git@gitlab.mycompany.com:2222/team/project.git')
      ).toEqual({
        provider: 'gitlab',
        host: 'gitlab.mycompany.com',
        projectPath: 'team/project',
